...
class StandardROIHeadsPseudoLab(StandardROIHeads):
    ...
    def _forward_box(
        ...
        if branch == "mixup":
            box_features = self.box_pooler(features, [x.gt_boxes for x in proposals])
            ground_truth = torch.cat([x.gt_classes for x in proposals])
            return box_features, ground_truth
        if branch == "teacher_guide":
            box_features_256 = self.box_pooler(features, [x.proposal_boxes for x in proposals])
            box_features = self.box_head(box_features_256)
            predictions = self.box_predictor(box_features)
            pred_instances, _ = self.box_predictor.inference(predictions, proposals)
            return box_features_256, pred_instances, predictions
        if branch == "prototype":
            box_features = self.box_pooler(features, [x['instances'].pred_boxes for x in proposals])
