...
def add_ubteacher_config(cfg):
    ...
    _C.DATASETS.TRAIN_LABEL = ("my_train",)
    _C.DATASETS.TRAIN_UNLABEL = ("my_train",)
    ...
    _C.SEMISUPNET.EXEMPLAR_MINING = False
    _C.SEMISUPNET.EXEMPLAR_REPLAY = False
    _C.SEMISUPNET.MIXUP = False
    _C.SEMISUPNET.SCENE_LEVEL = False
    _C.SEMISUPNET.INSTANCE_LEVEL = False
    _C.SEMISUPNET.LOGIT_LEVEL = False
    _C.SEMISUPNET.POST = False

    # dataloader
    # supervision level
    _C.DATALOADER.NAME_OLD_CLASSES = []
    _C.DATALOADER.NAME_NEW_CLASSES = []
    _C.DATALOADER.SUP_PERCENT = 100.0  # 5 = 5% dataset as labeled set
    _C.DATALOADER.RANDOM_DATA_SEED = 0  # random seed to read data
    _C.DATALOADER.RANDOM_DATA_SEED_PATH = "dataseed/SRSDD_supervision.txt"
